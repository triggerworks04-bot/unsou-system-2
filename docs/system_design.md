# 運行システム2 — システム全体設計

## 1. 結論（設計の要）

- **データの起点（システム連携）**は **`10_配車予定`**（月別の印刷用・横持ち配車表から変換した**縦持ちDB**。詳細は下記「二層構造」）。付与される **運行予定ID（内部Key: UNIQUEID）** を点呼・日報・売上・カレンダー・LINEと紐づける。**予定変更時は同一内部Keyを維持**し、**ステータス**と**更新日時**で管理する。
- **実体DB** は Google スプレッドシート。現場入力は **AppSheet**。**採番**は内部Keyを **UNIQUEID()** とし、人が読む **表示用番号** は別列（GAS／式で後から整備可）。
- **単独運用**：各サブシステム用シート／アプリだけでも業務が回る。**連携**：同一マスタIDと運行予定IDで結合可能。
- **補助機能**として **LINE通知**（キュー＋重複防止）を設計する。詳細は `line_notification_design.md`。
- **列仕様の正本**は `schema_master.md`。Google Drive 資料は参照のみ。

## 2. スコープ

| 名称 | 位置づけ |
|------|----------|
| **10_配車予定**（システム連携側） | 上流の**論理データ**。**1日・1案件・1担当者・1車両＝1行**の縦持ち。**ステータス**ほかによりカレンダー・LINE・点呼・日報・売上と連携。生成元は月別印刷用横持ち表→**変換処理** |
| 月別配車表（印刷用） | `配車予定表_2026年マスター.xlsx` 等。**A3・掲示・人間の配車調整**用。システム管理列は原則載せない。**カレンダー／LINE等へは直接連携しない** |
| 点呼記録簿 | 点呼タイミング・記録 |
| 運行日報 | 当日実績。**日報ヘッダ**（1日単位など）と **運行明細**（便・実出発／実到着・区間地表示）に分離 |
| 売上管理表 | 案件・運行に基づく売上（**運行日報ID** で計上根拠を辿れるようにする） |
| LINE通知（補助） | 翌日予定通知等、`schema_master.md` の LINE通知キューで管理 |
| **11_案件候補**（設計上の追加） | **画像取込・手入力・定期生成前**の候補を一時保存（**33列**）。**検討案件はここで管理**し、**決定後のみ 10_配車予定 に反映**。詳細は `schema_master.md` 節 B |
| **09_定期案件テンプレート**（設計上の追加） | **定期マスタ**（**24列**）。月次（運用で定める単位）で **10_配車予定** に生成する想定（実装は後続） |
| **90_配車表表示用**（設計上の追加） | **A3 印刷・PDF 閲覧用**の派生データ（**13列**）。**入力の正本ではない**（`schema_master.md` 節 B-5） |

Google Drive にある Excel／PDF／雛形は **参照・移行元** とし、論理モデルは本ドキュメントとスキーママスタで管理する。

## 配車予定表の二層構造

運行システム2では、配車予定表を以下の**二層**で扱う。

### 1. 印刷・入力・確認用の月別配車表

- 現在の **配車予定表_2026年マスター.xlsx** の**横持ち**形式を維持する。
- **A3** 印刷して事務所に掲示する。
- 配車担当者が全体を見ながら予定を調整する。
- **人が見やすい**ことを優先する。
- **運行予定ID**、**カレンダーイベントID**、**LINE送信済みフラグ** などの**システム管理列は原則として載せない**（増やしすぎない）。

### 2. システム連携用の 10_配車予定

- 月別配車表から**変換**して作成する**縦持ちDB**（スプレッドシート上のシート名の例）。
- **1日・1案件・1担当者・1車両 ＝ 1行**とする。
- **Googleカレンダー連携**、**LINE通知**、**点呼記録簿**、**運行日報**、**売上管理表**は、この **10_配車予定** を起点に連携する。

### データの流れ（初期運用）

**完全自動**を前提にせず、まずは**変換処理を実行**して **10_配車予定** を更新する方式とする。

```
月別配車表
  → 変換処理（実行トリガ）
  → 10_配車予定
  → Googleカレンダー / LINE通知 / 点呼記録簿 / 運行日報 / 売上管理表
```

**変換の詳細ルール**（読み取り単位・空欄・照合・重複防止など）は **`docs/dispatch_conversion_design.md`** を参照する。**GAS での関数構成・入出力シート・UPSERT／更新ルール・初期実装スコープ**は **`docs/dispatch_conversion_gas_spec.md`** を参照する。実装は **GAS** とし、初版は手動またはボタン実行を想定する。

### 案件候補・定期・表示レイヤ（方針のみ）

- **10_配車予定** は引き続き **システム連携上の正本**（運行予定ID をキーに点呼・日報・売上等へ）。**既存の縦持ち・変換前提は変更しない**（列は **`schema_master.md`** で **追加**）。
- **画像あり／画像なしの単発**は **`11_案件候補`** に取り込み、**決定した行だけ** を **10** に反映する。**検討案件を 10 に直接増やさない**。
- **定期**は **`09_定期案件テンプレート`** をマスタとし、**10** へ月次（等）生成する流れを想定する（アルゴリズム・画面は後続）。
- **`90_配車表表示用`** は **印刷・閲覧用**。**マスタ入力や案件確定の起点にはしない**。**10**・マスタから合成する。
- **将来**：**11** の空き照会系の列を起点に、空き運転者・空き車両照会へ接続する想定。
- **AppSheet**：現フェーズでは **画面は作らず**、スキーマ確定後に Regenerate する。運用方針のメモは **`docs/appsheet_settings.md`**。
- **列数（新規シート）**：**09_定期案件テンプレート＝24列**、**11_案件候補＝33列**、**90_配車表表示用＝13列**（ヘッダー順・型は **`schema_master.md`** 節 B・**B-6**）。

## 3. データ連携モデル（概念）

```mermaid
flowchart TB
  subgraph masters [共通マスタ]
    D[運転者ID]
    V[車両ID]
    P[案件ID]
    C[取引先ID]
  end
  PRINT[月別配車表 横持ち A3／入力]
  XFORM[変換処理 初版は手動またはGAS実行]
  CAL[10_配車予定 縦持ちDB]
  subgraph downstream [下流・単独可]
    T[点呼記録簿]
    DRH[日報ヘッダ]
    DRR[運行明細]
    S[売上管理表]
  end
  subgraph aux [補助]
    LNQ[LINE通知キュー]
    GC[Googleカレンダー]
  end
  PRINT --> XFORM --> CAL
  CAL -->|運行予定ID| T
  CAL -->|運行予定ID| DRH
  CAL -->|運行予定ID| DRR
  CAL -->|運行予定ID| S
  DRH -->|日報ID| DRR
  DRH -->|運行日報ID| S
  CAL <-->|イベント同期| GC
  CAL <-->|キュー生成| LNQ
  masters -.-> CAL
  masters -.-> T
  masters -.-> DRH
  masters -.-> S
```

## 4. 共通IDの役割

| ID | 用途 |
|----|------|
| **運行予定ID** | 1件の「予定された運行」の内部Key（UNIQUEID）。横断結合の主軸。**変更しても同一Key維持（原則）**。 |
| **運行予定番号等** | 表示用。**帳票・人間向け**。 `schema_master.md` の「ID・採番ルール」参照。 |
| **運転者ID** | 運転者マスタの Key。 |
| **車両ID** | 車両マスタの Key。 |
| **案件ID** | 案件マスタの Key。 |
| **取引先ID** | 取引先マスタの Key。 |
| **案件候補ID** | **11_案件候補** の内部Key（UNIQUEID）。 |
| **定期案件ID** | **09_定期案件テンプレート** の内部Key（UNIQUEID）。 |
| **配車予定ID（論理名）** | **10_配車予定** の 1 行を指す。**物理列は当面「運行予定ID」を正とし**、`schema_master.md` 節 B-0 のとおり **別列「配車予定ID」は 10 に増やさない**（**90** の Ref 列名のみ「配車予定ID」としてよい）。 |

**運行日報の運行予定ID**：ヘッダは「代表として起票した **10_配車予定** 行」、明細は「その便が対応する **10_配車予定**」。**実績集計・売上連携では明細側を優先**（同一日・単一予定では多く同一値）。`schema_master.md` の「運行日報における運行予定ID」、`daily_report_design.md` を参照。

## 5. Google Drive 資料との関係

- Drive 上のファイルは「現場で使っている物」「入力雛形」「集計」の**参考資料**として登録する（一覧は `source_materials.md`）。
- **設計・列の決定権**は **`schema_master.md`** および **`docs/`** におく。

## 6. 今後の拡張ポイント

- GAS と AppSheet の責務分割（送信・同期・表示用番号採番）。
- LINE 公式／LINE WORKS の選定。
- 権限・監査ログの要否。

詳細は各サブ設計書に委ねる。
